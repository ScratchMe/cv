...
- **Données structurées** (JSON-LD, écrites par le pré-rendu depuis `scripts/lib/structured-data.js`, dans la langue de chaque page ; la personne a partout le même `@id`, `https://cv.antoine.berthaud.me/#person`) :
  - accueil : `ProfilePage` → `Person` (métier, lieu, formation, sujets maîtrisés, `sameAs` vers LinkedIn et le portfolio photo), avec `dateCreated` (premier commit de `index.html`) et `dateModified` (le `lastmod` de la page) ;
